...
function renderTermlyPapers() {
  ...
  grid.innerHTML = sortedGroups.map(group => {
    ...
    const paperRows = group.papers.map(p => {
      ...
      let actionBtns;
      if (isPlatformAdmin) {
        actionBtns = `<button class="btn btn-sm btn-primary" onclick="downloadTermlyPaper('${p.id}')">&#11015;&#65039; Download</button>` +
          `<button class="btn btn-sm" style="background:#fee2e2;color:#dc2626;border:none" onclick="deleteTermlyPaper('${p.id}')">&#128465;</button>`;
      } else if (isFree) {
        actionBtns = `<button class="btn btn-sm btn-primary" onclick="downloadTermlyPaper('${p.id}')">&#11015;&#65039; Download Free</button>`;
      } else {
        actionBtns = `<button class="btn btn-sm" style="background:linear-gradient(135deg,#1a6fb5,#7c3aed);color:#fff;border:none;font-weight:700;padding:.4rem .85rem" onclick="downloadTermlyPaper('${p.id}')">&#128179; Buy &amp; Download &mdash; KES ${p.price.toLocaleString()}</button>`;
      }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (settingsSec) {
    ...
    apiCard.innerHTML = `
      <h3 style="padding:.75rem 1.25rem .5rem;font-size:1rem">🤖 Exam Builder — AI Settings</h3>
      <div style="padding:.5rem 1.25rem 1rem">
        <div style="padding:.5rem .75rem;background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.3);border-radius:8px;font-size:.79rem;color:#065f46;margin-bottom:.85rem">
          ✅ <strong>AI works out of the box</strong> inside Charanas Analyzer — no API key needed. The API key below is only needed if you use this app outside claude.ai.
        </div>
        <div class="fg" style="margin-bottom:.75rem">
          <label>Anthropic API Key (optional)</label>
          <input type="password" id="ebApiKeyInput" placeholder="sk-ant-... (only needed outside claude.ai)" style="width:100%;padding:8px 12px;border:1.5px solid var(--border);border-radius:8px;font-size:.9rem;outline:none"/>
          <div style="font-size:.76rem;color:var(--muted);margin-top:.3rem">Optional: only required when running the app as a standalone HTML file outside claude.ai.</div>
        </div>
        <button class="btn btn-primary btn-sm" onclick="ebSaveApiKey()">💾 Save API Key</button>
        <button class="btn btn-outline btn-sm" onclick="ebTestApiKey()" style="margin-left:.5rem">🔌 Test AI Connection</button>
